...
//  io.socket.request('/chat',{'forceNew': true });
    ...
    $('.js-add-chat').click(function(e){
        ...
        if(chatName){
            io.socket.post('/chat/addNewChat', {
                ...
                appendChatName(chat.name, chat.id);
            });
        }
    });
    ...
    $('.js-apply-chats').click(function(e){
        e.preventDefault();
        ...
        _.each($wrap.find('select').val(), function(item){
            if(item){
                userAllowed.push(+item);
            }
        });

        io.socket.post('/user/changeAllowedChats', {
            chatsAllowed: userAllowed,
            id: userId
        } ,function(resp){
            $elem.parents('.chats-update').hide().prev().show();
            if(resp.error){
                $('.js-logout').click();
                return false;
            }
            if(resp.chatsAllowed.length){
                var list = [];
                _.each(resp.chatsAllowed, function(item){
                    list.push(item.name);
                });

                $('.js-chats').text(list.join(', '));
            }
